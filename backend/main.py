...
def update_face(data: FaceData):
    system_state["face_detected"] = data.face_detected
    system_state["face_size"] = data.face_size

    result = check_alarm()

    return {"status": result, "alarm": system_state["alarm_active"]}
...
def update_phone(data: PhoneData):
    system_state["phone_detected"] = data.phone_detected
...
def update_site(data: SiteData):
    system_state["blocked_site_detected"] = data.blocked_site_detected
